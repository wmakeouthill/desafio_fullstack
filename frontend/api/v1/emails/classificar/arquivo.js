/**
 * Vercel Serverless Function - Proxy para endpoint de classificação de arquivo
 *
 * Esta função faz proxy específico para upload de arquivos,
 * lidando corretamente com multipart/form-data.
 */

const { GoogleAuth } = require('google-auth-library');
const getRawBody = require('raw-body');

// URL do backend no Cloud Run (privado)
const CLOUD_RUN_URL = process.env.CLOUD_RUN_URL || 'https://email-classifier-api-881402891442.southamerica-east1.run.app';

// Cache do token
let tokenCache = { token: null, expiry: 0 };

/**
 * Obtém o token de identidade do Google Cloud
 */
async function getIdToken() {
    const now = Date.now();

    if (tokenCache.token && tokenCache.expiry > now + 300000) {
        return tokenCache.token;
    }

    const credentials = JSON.parse(process.env.GOOGLE_SERVICE_ACCOUNT_KEY || '{}');

    if (!credentials.client_email) {
        throw new Error('GOOGLE_SERVICE_ACCOUNT_KEY não configurada ou inválida');
    }

    const auth = new GoogleAuth({ credentials });
    const client = await auth.getIdTokenClient(CLOUD_RUN_URL);
    const headers = await client.getRequestHeaders();

    const token = headers.Authorization?.replace('Bearer ', '');

    tokenCache = {
        token,
        expiry: now + 3600000
    };

    return token;
}

/**
 * Desabilitar body parser do Vercel para receber raw body
 * (usando sintaxe CommonJS para compatibilidade)
 */
module.exports.config = {
    api: {
        bodyParser: false,
    },
};

/**
 * Handler para upload de arquivos
 */
module.exports = async (req, res) => {
    // CORS
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type');

    if (req.method === 'OPTIONS') {
        return res.status(200).end();
    }

    if (req.method !== 'POST') {
        return res.status(405).json({ error: 'Método não permitido' });
    }

    try {
        // Obter token de autenticação
        const idToken = await getIdToken();

        // Obter query params (como provider)
        const url = new URL(req.url, `http://${req.headers.host}`);
        const provider = url.searchParams.get('provider');

        // Construir URL de destino
        let targetUrl = `${CLOUD_RUN_URL}/api/v1/emails/classificar/arquivo`;
        if (provider) {
            targetUrl += `?provider=${provider}`;
        }

        console.log(`[Upload Proxy] POST ${targetUrl}`);

        // Obter raw body
        const rawBody = await getRawBody(req);

        // Fazer requisição para Cloud Run
        const response = await fetch(targetUrl, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${idToken}`,
                'Content-Type': req.headers['content-type'],
                'Content-Length': rawBody.length.toString(),
            },
            body: rawBody,
        });

        const responseData = await response.text();

        res.setHeader('Content-Type', response.headers.get('content-type') || 'application/json');
        res.status(response.status);

        try {
            const jsonData = JSON.parse(responseData);
            return res.json(jsonData);
        } catch {
            return res.send(responseData);
        }

    } catch (error) {
        console.error('[Upload Proxy Error]', error);
        return res.status(500).json({
            error: 'Erro no upload',
            message: error.message,
            timestamp: new Date().toISOString()
        });
    }
};
